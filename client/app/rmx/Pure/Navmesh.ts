...
module rmx.Pure {
    ...
    export class HalfEdgeMesh {
        ...
        findEdgeBruteForce(begin, end): HalfEdgeEdge {
            return rmx.List.find(this.edges, function(edge) {
        ...
        findVertexAt(position: Vec3, tolerance: number): HalfEdgeVertex {
            return rmx.List.find(this.vertices, function(vertex) {
                var dx, dy, dz;
        ...
        findEdgeAt(position, tolerance, minT, maxT): HalfEdgeEdge {
            return rmx.List.find(this.edges, function(edge: HalfEdgeEdge) {
        ...
        stitchFaceBorder(face: HalfEdgeFace, toleranceDist, toleranceT) {
            ...
            while (vertices.length > 0) {
                v = vertices.shift();
                edge = rmx.List.find(mesh.edges, function(e: HalfEdgeEdge) {
            ...
            while (edges.length > 0) {
                e = edges.shift();
                vertex = rmx.List.find(mesh.vertices, function(v: HalfEdgeVertex) {
                    return e.isPointOnEdge(v.pos, toleranceDist, toleranceT, 1 - toleranceT);
                });
